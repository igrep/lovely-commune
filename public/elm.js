...
Elm.Position.make = function (_elm) {
   ...
   $Native$Position = Elm.Native.Position.make(_elm),
   $Result = Elm.Result.make(_elm),
   $Signal = Elm.Signal.make(_elm),
   $Task = Elm.Task.make(_elm),
   $Touch = Elm.Touch.make(_elm);
   var _op = {};
   var noOp = $Task.succeed({ctor: "_Tuple0"});
   var toPair = function (t) {
      return {ctor: "_Tuple2",_0: t.x,_1: t.y};
   };
   var mouseTouch = function () {
      var touch = A2($Signal.map,
      function (_p0) {
         return A2($Maybe.map,toPair,$List.head(_p0));
      },
      $Touch.touches);
      var mouse = A2($Signal.map,$Maybe.Just,$Mouse.position);
      return A2($Signal.merge,mouse,touch);
   }();
   var convertViewportPointToSvgPoint = function (xy) {
      return $Native$Position.convertViewportPointToSvgPoint(xy);
   };
   var getIdFromPoint = function (xy) {
      return $Native$Position.getIdFromPoint(xy);
   };
   var PointedSvgElementInfo = F2(function (a,b) {
      return {svgPosition: a,id: b};
   });
   var pointedSvgElementInfos = $Signal.mailbox(A2(PointedSvgElementInfo,
   {ctor: "_Tuple2",_0: 0,_1: 0},
   ""));
   var getPointedSvgElement = function (maybePosition) {
      var _p1 = maybePosition;
      if (_p1.ctor === "Just") {
            var _p3 = _p1._0;
            ...
            function (maybeId) {
               ...
               if (_p2.ctor === "Just") {
                     return A2($Task.andThen,
                     convertViewportPointToSvgPoint(_p3),
                     function (svgPosition) {
                        return $Task.succeed($Maybe.Just(A2(PointedSvgElementInfo,
                        svgPosition,
                        _p2._0)));
                     });
                  } else {
   ...
   var sendPointedSvgElement = F2(function (address,
   maybePosition) {
      return A2($Task.andThen,
      getPointedSvgElement(maybePosition),
      function (maybeElement) {
         var _p4 = maybeElement;
         if (_p4.ctor === "Just") {
               return A2($Signal.send,address,_p4._0);
            } else {
               return noOp;
            }
      });
   });
   var keepSendingPointedSvgElementInfo = A2($Signal.map,
   sendPointedSvgElement(pointedSvgElementInfos.address),
   mouseTouch);
   return _elm.Position.values = {_op: _op
                                 ,pointedSvgElementInfos: pointedSvgElementInfos
                                 ,keepSendingPointedSvgElementInfo: keepSendingPointedSvgElementInfo
                                 ,PointedSvgElementInfo: PointedSvgElementInfo};
};
Elm.Heart = Elm.Heart || {};
Elm.Heart.Part = Elm.Heart.Part || {};
Elm.Heart.Part.Constants = Elm.Heart.Part.Constants || {};
Elm.Heart.Part.Constants.make = function (_elm) {
   "use strict";
   _elm.Heart = _elm.Heart || {};
   _elm.Heart.Part = _elm.Heart.Part || {};
   _elm.Heart.Part.Constants = _elm.Heart.Part.Constants || {};
   if (_elm.Heart.Part.Constants.values) return _elm.Heart.Part.Constants.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $List = Elm.List.make(_elm),
   ...
   var empty = 0.0;
   var filled = 1.0;
   return _elm.Heart.Part.Constants.values = {_op: _op
                                             ,filled: filled
                                             ,empty: empty
                                             ,left: left
                                             ,right: right
...
Elm.Heart = Elm.Heart || {};
Elm.Heart.Part = Elm.Heart.Part || {};
Elm.Heart.Part.make = function (_elm) {
   "use strict";
   _elm.Heart = _elm.Heart || {};
   _elm.Heart.Part = _elm.Heart.Part || {};
   if (_elm.Heart.Part.values) return _elm.Heart.Part.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Heart$Part$Constants = Elm.Heart.Part.Constants.make(_elm),
   $List = Elm.List.make(_elm),
   $Maybe = Elm.Maybe.make(_elm),
   ...
   var _op = {};
   var axisY = $Basics.snd;
   var axisX = $Basics.fst;
   var stopId = function (m) {
      return A2($Basics._op["++"],"gradientStop-",m.id);
   };
   var gradientId = function (m) {
      return A2($Basics._op["++"],"gradient-",m.id);
   };
   var viewGradient = function (m) {
      return A2($Svg.linearGradient,
      _U.list([$Svg$Attributes.spreadMethod("pad")
              ,$Svg$Attributes.gradientUnits("userSpaceOnUse")
              ,$Svg$Attributes.x1($Basics.toString(axisX(m.gradientEnd1)))
              ,$Svg$Attributes.y1($Basics.toString(axisY(m.gradientEnd1)))
              ,$Svg$Attributes.x2($Basics.toString(axisX(m.gradientEnd2)))
              ,$Svg$Attributes.y2($Basics.toString(axisY(m.gradientEnd2)))
              ,$Svg$Attributes.id(gradientId(m))]),
      _U.list([A2($Svg.stop,
              _U.list([$Svg$Attributes.$class("heart-gradiationStop heart-gradiationStop--traced_true")
                      ,$Svg$Attributes.offset($Basics.toString(m.filled))]),
              _U.list([]))
              ,A2($Svg.stop,
              _U.list([$Svg$Attributes.$class("heart-gradiationStop heart-gradiationStop--traced_false")
                      ,$Svg$Attributes.offset($Basics.toString(m.filled))]),
              _U.list([]))]));
   };
   var viewPath = function (m) {
      var $class = m.isPulsating ? A2($Basics._op["++"],
      "heart-part",
      " animated flash") : "heart-part";
      return A2($Svg.path,
      _U.list([$Svg$Attributes.$class($class)
              ,$Svg$Attributes.d(m.draw)
              ,$Svg$Attributes.id(m.id)
              ,$Svg$Attributes.style(A2($Basics._op["++"],
              "fill: url(#",
              A2($Basics._op["++"],gradientId(m),")")))]),
      _U.list([]));
   };
   var isFilled = function (m) {
      return _U.cmp(m.filled,$Heart$Part$Constants.filled) > -1;
   };
   var pulsate = function (m) {
      return _U.update(m,
      {isPulsating: true,filled: $Heart$Part$Constants.filled});
   };
   var traceAt = F3(function (_p1,_p0,m) {
      return _U.update(m,{filled: $Heart$Part$Constants.filled});
   });
   var FromBottomRight = {ctor: "FromBottomRight"};
   var FromTopLeft = {ctor: "FromTopLeft"};
   var Model = F7(function (a,b,c,d,e,f,g) {
      return {filled: a
             ,isPulsating: b
             ,id: c
             ,positionComparator: d
             ,gradientEnd1: e
             ,gradientEnd2: f
             ,draw: g};
   });
   var init = A2(Model,$Heart$Part$Constants.empty,false);
   var initLeft = A5(init,
   $Heart$Part$Constants.left,
   axisY,
   {ctor: "_Tuple2",_0: 78,_1: 73},
   {ctor: "_Tuple2",_0: 123,_1: 395},
   ...
   var initRight = A5(init,
   $Heart$Part$Constants.right,
   axisY,
   {ctor: "_Tuple2",_0: 501,_1: 73},
   {ctor: "_Tuple2",_0: 457,_1: 395},
   ...
   var initUpperLeft = A5(init,
   $Heart$Part$Constants.upperLeft,
   axisX,
   {ctor: "_Tuple2",_0: 88,_1: 100},
   {ctor: "_Tuple2",_0: 291,_1: 100},
   ...
   var initUpperRight = A5(init,
   $Heart$Part$Constants.upperRight,
   axisX,
   {ctor: "_Tuple2",_0: 290,_1: 100},
   {ctor: "_Tuple2",_0: 492,_1: 100},
   ...
   var initBottomLeft = A5(init,
   $Heart$Part$Constants.bottomLeft,
   axisX,
   {ctor: "_Tuple2",_0: 149,_1: 340},
   {ctor: "_Tuple2",_0: 289,_1: 517},
   ...
   var initBottomRight = A5(init,
   $Heart$Part$Constants.bottomRight,
   axisX,
   {ctor: "_Tuple2",_0: 290,_1: 407},
   {ctor: "_Tuple2",_0: 446,_1: 407},
   ...
   var initCenterLeft = A5(init,
   $Heart$Part$Constants.centerLeft,
   axisX,
   {ctor: "_Tuple2",_0: 155,_1: 273},
   {ctor: "_Tuple2",_0: 291,_1: 273},
   ...
   var initCenterRight = A5(init,
   $Heart$Part$Constants.centerRight,
   axisX,
   {ctor: "_Tuple2",_0: 290,_1: 273},
   {ctor: "_Tuple2",_0: 426,_1: 273},
   ...
   var initModels = _U.list([initLeft
                            ,initRight
                            ,initUpperLeft
                            ,initUpperRight
                            ,initBottomLeft
                            ...
                            ,initCenterLeft
                            ,initCenterRight]);
   return _elm.Heart.Part.values = {_op: _op
                                   ,Model: Model
                                   ...
                                   ,initLeft: initLeft
                                   ,initRight: initRight
                                   ,initUpperLeft: initUpperLeft
                                   ,initUpperRight: initUpperRight
                                   ,initBottomLeft: initBottomLeft
                                   ,initBottomRight: initBottomRight
                                   ,initCenterLeft: initCenterLeft
                                   ,initCenterRight: initCenterRight
                                   ,axisX: axisX
                                   ,axisY: axisY};
};
Elm.Heart = Elm.Heart || {};
Elm.Heart.make = function (_elm) {
   "use strict";
   _elm.Heart = _elm.Heart || {};
   if (_elm.Heart.values) return _elm.Heart.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Dict = Elm.Dict.make(_elm),
   $Effects = Elm.Effects.make(_elm),
   $Heart$Part = Elm.Heart.Part.make(_elm),
   $Heart$Part$Constants = Elm.Heart.Part.Constants.make(_elm),
   $Html = Elm.Html.make(_elm),
   $List = Elm.List.make(_elm),
   $Maybe = Elm.Maybe.make(_elm),
   ...
   var join = function (x) {
      return function (_p0) {
         return $List.concat(A2($List.intersperse,x,_p0));
      };
   };
   var view = F2(function (_p1,m) {
      var parts = $Dict.values(m.heartState);
      var contents = A2($Basics._op["++"],
      _U.list([A2($Svg.defs,
      _U.list([]),
      A2($List.map,$Heart$Part.viewGradient,parts))]),
      A2($List.map,$Heart$Part.viewPath,parts));
      return A2($Svg.svg,
      _U.list([$Svg$Attributes.version("1.1")
              ,$Svg$Attributes.width("100%")
              ,$Svg$Attributes.height("100%")
              ,$Svg$Attributes.viewBox("0 0 574.49699 527.37143")]),
      contents);
   });
   var affectEventually = F2(function (duration,a) {
   ...
   var popDirection = function (m) {
      var _p3 = m.leftDirections;
      if (_p3.ctor === "::") {
            return _U.update(m,{leftDirections: _p3._1});
         } else {
            return m;
         }
   };
   var tracePart = F3(function (pointedSvgElementInfo,
   traceFrom,
   m) {
      var pointedId = pointedSvgElementInfo.id;
      var tracedM = _U.update(m,
      {heartState: A3($Dict.update,
      pointedId,
      $Maybe.map(A2($Heart$Part.traceAt,
      pointedSvgElementInfo.svgPosition,
      traceFrom)),
      m.heartState)});
      var _p4 = A2($Dict.get,pointedId,tracedM.heartState);
      if (_p4.ctor === "Just") {
            return $Heart$Part.isFilled(_p4._0) ? popDirection(tracedM) : tracedM;
         } else {
            return tracedM;
         }
   });
   var turnedOffHeartState = $Dict.fromList(A2($List.map,
   function (m) {
      return {ctor: "_Tuple2",_0: m.id,_1: m};
   },
   $Heart$Part.initModels));
   var pulsateCenter = function (m) {
      var m$ = _U.update(m,{heartState: turnedOffHeartState});
      var m$$ = _U.update(m$,
      {heartState: A3($Dict.update,
      $Heart$Part$Constants.centerLeft,
      $Maybe.map($Heart$Part.pulsate),
      m$.heartState)});
      return _U.update(m$$,
      {heartState: A3($Dict.update,
      $Heart$Part$Constants.centerRight,
   ...
   var Direction = F2(function (a,b) {
      return {targetId: a,traceFrom: b};
   });
   var cL = _U.list([A2(Direction,
                    $Heart$Part$Constants.left,
                    $Heart$Part.FromTopLeft)
                    ,A2(Direction,
                    $Heart$Part$Constants.bottomLeft,
                    $Heart$Part.FromTopLeft)
                    ,A2(Direction,
                    $Heart$Part$Constants.bottomRight,
                    $Heart$Part.FromTopLeft)]);
   var cO = _U.list([A2(Direction,
                    $Heart$Part$Constants.bottomLeft,
                    $Heart$Part.FromBottomRight)
                    ,A2(Direction,
                    $Heart$Part$Constants.left,
                    $Heart$Part.FromBottomRight)
                    ,A2(Direction,
   ...
   var cE = _U.list([A2(Direction,
                    ...
                    $Heart$Part$Constants.bottomRight,
                    $Heart$Part.FromTopLeft)]);
   var directionStack = A2(join,
   _U.list([$Maybe.Nothing]),
   A2($List.map,$List.map($Maybe.Just),_U.list([cL,cO,cV,cE])));
   var init = {ctor: "_Tuple2"
              ,_0: {heartState: turnedOffHeartState
                   ,leftDirections: directionStack}
              ,_1: $Effects.none};
   var Complete = {ctor: "Complete"};
   var pulateEventually = A2(affectEventually,900,Complete);
   var Reset = {ctor: "Reset"};
   var resetEventually = A2(affectEventually,600,Reset);
   var update = F2(function (a,m) {
      var _p5 = a;
      switch (_p5.ctor)
      {case "Reset": return {ctor: "_Tuple2"
                            ,_0: _U.update(m,{heartState: turnedOffHeartState})
                            ,_1: $Effects.none};
         case "Trace": var _p11 = _p5._0;
           var _p6 = m.leftDirections;
           if (_p6.ctor === "::") {
                 var _p10 = _p6._1;
                 var _p7 = _p6._0;
                 if (_p7.ctor === "Just") {
                       var _p9 = _p7._0;
                       if (_U.eq(_p11.id,_p9.targetId)) {
                             var tracedM = A3(tracePart,_p11,_p9.traceFrom,m);
                             var _p8 = _p10;
                             if (_p8.ctor === "::" && _p8._0.ctor === "Nothing") {
                                   return {ctor: "_Tuple2",_0: tracedM,_1: resetEventually};
                                } else {
                                   return {ctor: "_Tuple2",_0: tracedM,_1: $Effects.none};
                                }
                          } else return {ctor: "_Tuple2",_0: m,_1: $Effects.none};
                    } else {
                       return {ctor: "_Tuple2"
                              ,_0: _U.update(m,{leftDirections: _p10})
                              ,_1: $Effects.none};
                    }
              } else {
                 return {ctor: "_Tuple2",_0: m,_1: pulateEventually};
              }
         default: return {ctor: "_Tuple2"
                         ,_0: pulsateCenter(m)
                         ,_1: $Effects.none};}
   });
   var Trace = function (a) {    return {ctor: "Trace",_0: a};};
   var Model = F2(function (a,b) {
      return {heartState: a,leftDirections: b};
   });
   return _elm.Heart.values = {_op: _op
                              ,init: init
                              ,update: update
                              ,view: view
                              ,Trace: Trace
...
Elm.Main.make = function (_elm) {
   ...
   $Task = Elm.Task.make(_elm);
   var _op = {};
   var keepSendingPointedElementInfo = Elm.Native.Task.make(_elm).performSignal("keepSendingPointedElementInfo",
   $Position.keepSendingPointedSvgElementInfo);
   var traces = A2($Signal.map,
   $Heart.Trace,
   $Position.pointedSvgElementInfos.signal);
   var app = $StartApp.start({init: $Heart.init
                             ,update: $Heart.update
                             ,view: $Heart.view
                             ,inputs: _U.list([traces])});
   var main = app.html;
